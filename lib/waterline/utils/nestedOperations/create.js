...
module.exports = function(parentModel, associations, cb) {
  var self = this;
  ...
  _.keys(associations).forEach(function(association) {
    ...
    var related = self.offshore.collections[modelName];
    var relatedPK = _.find(related.attributes, { primaryKey: true });

    // Get the attribute's name
    var pk = self.offshore.collections[modelName].primaryKey;
  ...
  parentModel._loadQuery(self._query).save(cb);
};
