...
Add.prototype.createAssociations = function(key, records, cb) {
  ...
  if (this.collection._validator && this.collection._validator.associations[key]) {
    associatedCollection = _.extend(new associatedCollection.constructor(associatedCollection.offshore, associatedCollection.connections), associatedCollection)._loadQuery(self.collection._query);
...
Add.prototype.createNewRecord = function(collection, attribute, values, key, cb) {
  ...
  collection.create(values, function(err, record) {
    ...
    if (self._query) {
      joinCollection = joinCollection.__loadQuery(self._query);
    }
...
Add.prototype.createManyToMany = function(collection, attribute, pk, key, cb) {
  ...
  async.auto({
    ...
    validateAssociation: function(next) {
      ...
      if (self._query) {
        associatedCollection = associatedCollection.__loadQuery(self._query);
      }
