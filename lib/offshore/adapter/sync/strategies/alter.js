...
module.exports = function(cb) {
  ...
  self.describe(function afterDescribe(err, attrs) {
    ...
    var collectionName = _.find(self.query.offshore.schema, {tableName: self.collection}).identity;

    // Create a mapping of column names -> attribute names
    var columnNamesMap = _.reduce(self.query.offshore.schema[collectionName].attributes, function(memo, val, key) {
    ...
    //  every time you initialize Offshore.)
    //
    ...
    self.find(queryCriteria, function(err, existingData) {
      ...
      var attrs = self.query.offshore.collections[self.identity]._attributes;
      var pk = self.query.offshore.collections[self.identity].primaryKey;
        ...
        if (cleansedUniqueData.length < cleansedExistingData.length) {
          ...
          prompt.get(['y/n'], function(err, results) {
            ...
            if (wasConfirmedByUser) {
              ...
              // throw new Error();
              // self.query.offshore.collections[self.collection].destroy(destroyCriteria).exec(each_cb);
...
function uhoh(err, backupData, cb) {
  ...
  console.error('Offshore encountered a fatal error when trying to perform the `alter` auto-migration strategy.');
