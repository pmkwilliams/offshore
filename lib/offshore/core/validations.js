 ...
 * https://github.com/balderdashy/offshore-validator
 */
...
Validator.prototype.initialize = function(attrs, types, defaults) {
  ...
  // add custom type definitions to offshore-validator
  types = types || {};
  offshore-validator.define(types);
...
Validator.prototype.validate = function(values, presentOnly, cb) {
  ...
  function validate(validation, cb) {
    ...
    // Build Requirements
    var requirements = _.cloneDeep(offshore-validator(curValidation));
    ...
    async.each(Object.keys(requirements.data), function(key, cb) {
      ...
      var err = offshore-validator(value).to(requirements.data, values);
