  ...
  for (var key in this.attributes) {
    if ((!hop(values, key) || values[key] === undefined) && hop(this.attributes[key], 'defaultsTo')) {
      ...
      if (typeof defaultsTo === 'function') {
        ...
        values[key] = _.clone(defaultsTo, function dealWithBuffers(val, key) {
...
function createValues(valuesObject, cb) {
  ...
  self.adapter.loadQuery(self.transaction).create(valuesObject.values, function(err, values) {
    if (err) {
    ...
    if (_.keys(valuesObject.associations.collections).length === 0) return after(values);
