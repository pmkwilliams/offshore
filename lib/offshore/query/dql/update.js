...
function prepareArguments(criteria, values) {
  ...
  values = nestedOperations.reduceAssociations.call(this, this.identity, this.offshore.schema, values, 'update');
...
function createBelongsTo(valuesObject, cb) {
  ...
  async.each(_.keys(valuesObject.associations.models), function(item, next) {
    ...
    var model = self.offshore.collections[modelName];
    ...
    query.exec(function(err, val) {
      ...
      delete valuesObject.associations.models[item];
      next();
    });
...
function updateRecords(valuesObject, cb) {
  ...
  // Pass to adapter
  self.adapter.loadQuery(self.transaction).update(valuesObject.criteria, valuesObject.values, function(err, values) {
    if (err) {
...
function updatedNestedAssociations(valuesObject, values, cb) {
  ...
  associations.models = _.pick(associations.models, function(vals) {
