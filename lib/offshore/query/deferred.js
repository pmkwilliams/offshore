...
Deferred.prototype.populate = function(keyName, criteria) {
  ...
  try {
    ...
    if(this._context.offshore.schema[attr.references].junctionTable) join.select = false;

    joins.push(join);

    // If a junction table is used add an additional join to get the data
    if(this._context.offshore.schema[attr.references].junctionTable && hasOwnProperty(attr, 'on')) {

      // clone the reference attribute so we can mutate it
      var reference = _.clone(this._context.offshore.schema[attr.references].attributes);
      ...
      if(ref) {
        ...
        var selects = _.map(_.keys(this._context.offshore.schema[reference[ref].references].attributes), function(attr) {
          var expandedAttr = self._context.offshore.schema[reference[ref].references].attributes[attr];
