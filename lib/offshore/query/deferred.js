...
Deferred.prototype.whereDeep = function(where, cb) {
  var self = this;
  var values = _.clone(where);
  var collections = this._context.offshore.collections;
  var attributes = collections[this._context.identity].attributes;
  if (!where) {
    return cb(null, where);
  }
  async.map(_.keys(values), function(property, next) {
    // check if there is something to solve
	if ( attributes[property] && _.isPlainObject(values[property]) && (hasOwnProperty(attributes[property], 'model') || hasOwnProperty(attributes[property], 'collection')) ) {
      var modelName = attributes[property]['model'] || attributes[property]['collection'];
      var collection = collections[modelName];
      var pk = _.find(_.keys(collection.attributes), function(key) {
        return collection.attributes[key].primaryKey;
      });        
      collection.find(values[property], function(err, data) {
        if (err) {
          return next(err);
        }
        var pks = _.map(data, function(model) {
          return model[pk];
        });
        next(null, pks);
      });
	} else {
      next(null, values[property]);
    }
  }, function(err, res) {
    if (err) {
      return cb(err);
    }
	_.keys(values).forEach(function(key, index) {
      values[key] = res[index];
    });
    cb(null, values);
  });
};
...
Deferred.prototype.exec = function(cb) {
  ...
  var queryWhere = this._criteria ? this._criteria.where : null;
  this.whereDeep(queryWhere, function(err, where) {
