  ...
  async.eachSeries(_.keys(transactionCnx), function(cnx, cb) {
    ...
    _.keys(transactionCnx).forEach(function(cnx) {
      ...
      _.keys(connection.collections).forEach(function(col) {
        transaction[col] = Object.create(null);
        transaction[col].__proto__ = connection.context.collections[col];
        transaction[col]._query = transaction[col]._query || {};
        transaction[col]._query.transaction = transactionDictionary;
      });
    });
...
TransactionDeferred.prototype.commit = function(result) {
  ...
  async.eachSeries(_.keys(self._connections), function(cnx, next) {
    console.log('BLA : ', self);
    self._connections[cnx].adapter.commit(self.transactionDictionary[cnx], _.keys(self._connections[cnx].collections), next);
  }, function() {
...
TransactionDeferred.prototype.rollback = function(err) {
  ...
  this._rollback = err;
  async.eachSeries(_.keys(self._connections), function(cnx, next) {
    console.log('JIHELJHEF : ', self._connections);
