...
module.exports = function _join(options) {
  ...
  // Usage
  var invalid = false;
  invalid = invalid || offshore-validator(options).to({
    type: 'object'
  });
  ...
  invalid = invalid || offshore-validator(options.parent).to({
    type: 'array'
  });
  invalid = invalid || offshore-validator(options.child).to({
    type: 'array'
  });
  invalid = invalid || offshore-validator(options.parentKey).to({
    type: 'string'
  });
  invalid = invalid || offshore-validator(options.childKey).to({
    type: 'string'
  });
