...
module.exports = function integrate(cache, joinInstructions, primaryKey, cb) {
  ...
  // Ensure valid usage
  var invalid = false;
  invalid = invalid || offshore-validator(cache).to({ type: 'object' });
  invalid = invalid || offshore-validator(joinInstructions).to({ type: 'array' });
  invalid = invalid || offshore-validator(joinInstructions[0]).to({ type: 'object' });
  invalid = invalid || offshore-validator(joinInstructions[0].parent).to({ type: 'string' });
  invalid = invalid || offshore-validator(cache[joinInstructions[0].parent]).to({ type: 'object' });
